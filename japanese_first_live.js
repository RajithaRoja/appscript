/**
 * The function `doGet` retrieves data from a Google Sheets spreadsheet, processes it to create English
 * and Japanese question objects with rankings, and returns the result as a JSON response.
 * @returns The `doGet` function is returning a JSON object containing two main properties: "english"
 * and "japanese".
 */
function doGet() {
...
function Chartdata(answers, body, language) {
    ...
    let dataRows = opportunityScoreSortValue.slice(0, 11).map(function (value) {
        return [value.questionId, value.opportunityScore]; 
    });
